...
SELECT * FROM 
(SELECT Members.recommendedby AS r, concat (
Members.surname, ' ', Members.firstname) AS n
FROM Bookings
...
SELECT * FROM
(SELECT Facilities.name AS n, Members.mid AS m, 
month(Members.joindate)) AS j
FROM Bookings
JOIN Facilities ON Bookings.facid = Facilities.facid
JOIN Members ON Bookings.memid = Members.mid) ss
